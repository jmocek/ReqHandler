...
def import_from_file(filename):
    ...
    for req in reqs:
        ...
        # Add Product Version if not defined earlier
        new_pv = []
        print(Product.get_versions())
        for pv in product_versions:
            if float(pv) not in Product.get_versions():
                new_pv.append(pv)
        print(new_pv)
        for pv in new_pv:
            db.session.add(Product(author=current_user.username, version=pv))
            db.session.commit()
